...
import Picker from '../rmc-picker/Picker';
...
class DatePicker extends React.Component {
    constructor() {
        ...
        this.onValueChange = (values, index) => {
            const props = this.props;
        ...
        this.onScrollChange = (values, index) => {
            const props = this.props;
    ...
    render() {
        ...
        const { disabled, pickerPrefixCls, prefixCls, rootNativeProps, className, style, itemStyle, } = this.props;
        const multiStyle = Object.assign({ flexDirection: 'row', alignItems: 'center' }, style);
        return (<MultiPicker style={multiStyle} rootNativeProps={rootNativeProps} className={className} prefixCls={prefixCls} selectedValue={value} onValueChange={this.onValueChange} onScrollChange={this.onScrollChange}>
        {cols.map(p => (<Picker style={{ flex: 1 }} key={p.key} disabled={disabled} prefixCls={pickerPrefixCls} itemStyle={itemStyle}>
            {p.props.children.map(item => (<Picker.Item key={item.value} value={item.value}>
                {item.label}
              </Picker.Item>))}
          </Picker>))}
      </MultiPicker>);
